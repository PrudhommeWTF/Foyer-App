...
| `cardLogoSearch` | Chercher un logo de carte de fidélité en ligne | Foyer | oui / non | activé | — | Cartes | `FOYER_CARD_LOGO_SEARCH` |
| `mcpEnabled` | Ouvrir le serveur pour les assistants (MCP) | Foyer | oui / non | désactivé | — | Assistants | — |
...
- **Chercher un logo de carte de fidélité en ligne** (`cardLogoSearch`) : À la saisie d’une carte, l’application propose des logos tirés du nom de l’enseigne (via autocomplete.clearbit.com, sans clé ni compte). C’est une requête sortante qui envoie le nom saisi. Coupé, seul le monogramme (les initiales) est proposé.
- **Ouvrir le serveur pour les assistants (MCP)** (`mcpEnabled`) : Expose le point d’entrée /mcp pour qu’un assistant (Claude, ChatGPT, un client MCP) puisse lire et agir dans le foyer, au nom du membre dont il porte le jeton d’accès. Éteint (par défaut), /mcp répond « introuvable ». Les finances et les réglages restent hors de portée, quel que soit le réglage. Ouvrez des jetons dans « Mon compte ».
