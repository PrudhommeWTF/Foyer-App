...
export function render(): string {
  ...
  L.push('```');
  L.push('');
  L.push('## Exploitation, en ligne de commande');
  L.push('');
  L.push('L’application montre l’état du service, la place restante et les sauvegardes');
  L.push('(Paramètres → Exploitation). Les mêmes gestes depuis un terminal :');
  L.push('');
  L.push('```bash');
  L.push('# État du service');
  L.push('systemctl status foyer                 # LXC');
  L.push('docker compose ps foyer                # Docker');
  L.push('journalctl -u foyer -f                 # journal en direct');
  L.push('');
  L.push('# Place restante et poids des données');
  L.push('df -h /var/lib/foyer && du -sh /var/lib/foyer');
  L.push('');
  L.push('# Instantané cohérent de la base, sans arrêter le service');
  L.push('# (la base est en WAL : copier foyer.db à chaud donne une archive corrompue)');
  L.push('sqlite3 /var/lib/foyer/foyer.db "VACUUM INTO \'/var/lib/foyer/sauvegardes/manuel.db\'"');
  L.push('```');
  L.push('');
  L.push('**Restaurer** ne se fait pas depuis l’application : remplacer la base pendant que le');
  L.push('service l’utilise la corromprait. Service arrêté :');
  L.push('');
  L.push('```bash');
  L.push('systemctl stop foyer');
  L.push('cd /var/lib/foyer');
  L.push('cp foyer.db foyer.db.avant-restauration');
  L.push('cp sauvegardes/foyer-AAAA-MM-JJ-HHMM.db foyer.db');
  L.push('rm -f foyer.db-wal foyer.db-shm        # les journaux de l’ancienne base');
  L.push('chown foyer:foyer foyer.db');
  L.push('systemctl start foyer && journalctl -u foyer -n 30 --no-pager');
  L.push('```');
  L.push('');
  L.push('Un instantané emporte **tout ce qui est en base**, finances comprises, mais **ni les');
  L.push('fichiers ni les photos**, qui vivent à côté sur le disque. Pour une archive vraiment');
  L.push('complète, c’est le dossier de données entier qu’il faut prendre, service arrêté');
  L.push('(voir « Où c’est stocké » plus haut).');
  L.push('');
