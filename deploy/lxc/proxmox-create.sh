...
if [[ -n "${LOCAL_SRC}" && -d "${LOCAL_SRC}/backend" ]]; then
  ...
  pct exec "${CTID}" -- bash -c "SELF_UPDATE='${SELF_UPDATE:-true}' FOYER_VERSION='${FOYER_VERSION}' FOYER_SRC=/root/foyer-src bash /root/foyer-src/deploy/lxc/install.sh"
else
  ...
  pct exec "${CTID}" -- bash -c \
    "apt-get update -qq && apt-get install -y -qq git >/dev/null && \
     ...
     SELF_UPDATE='${SELF_UPDATE:-true}' FOYER_VERSION='${FOYER_VERSION:-}' bash /root/foyer-src/deploy/lxc/install.sh"
fi
