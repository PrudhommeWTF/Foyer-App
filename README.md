...
| ✅ **Tâches** | **Saisie en un champ** : on tape, on valide ; membres, date et heure, liste, catégorie et note se règlent dans une barre sous le champ, sans formulaire, sur l'écran comme depuis l'accueil. Affectation à **un, plusieurs ou aucun** membre (« le premier qui passe »), échéance datée avec heure, catégorie, note. **Listes typées** : tâches (l'affaire du jour), corvées, checklists (valise, fournitures, idées) qui vivent dans leur onglet sans peser sur la journée ; partagées ou **privées** (cachées, pas chiffrées) ; archivables ; **modèles** réutilisables (« En modèle », puis « Depuis un modèle »). **Récurrence** native, dans deux modes explicites : à date fixe (les poubelles du mardi) ou **après la réalisation** (le test de la piscine, une semaine après l'avoir fait, qu'il ait été fait samedi ou dimanche : pas de retard fictif). Toutes les N unités, certains jours de la semaine, fin de série, et une **souplesse** en jours pour le saisonnier (« vers le 15 avril »). Une série est une seule tâche qui avance et garde l'historique de ses réalisations ; modifier ou supprimer demande « cette occurrence ou toute la série ». **Rappels** par notification sur le téléphone (Web Push), réglés tâche par tâche (à l'heure, 1 h avant, la veille à 18 h, le matin à 9 h), et **notification quand quelqu'un vous affecte une tâche** ; une tâche sans responsable rappelle tout le foyer. Sur iPhone, l'application doit être ajoutée à l'écran d'accueil ; *Paramètres → Notifications* dit où en est chaque appareil, envoie un test et montre les derniers envois. **Liée au reste du foyer** : une tâche née d'une échéance ouvre son **contrat** en un tap ; un **document** se lie à la saisie (ou « En tâche » depuis le document) et s'ouvre en un tap ; quand le dernier article d'une liste de courses est pris, la tâche qui l'ouvre propose de se **clore** ; au choix de la date, l'**emploi du temps** des membres affectés est lu (ce qu'ils ont ce jour-là, conflit à l'heure choisie, jour le plus libre en un tap), jamais modifié ; les tâches datées sont dans l'**agenda** (tap pour ouvrir) et, sur option, dans le **flux ICS**. **Sous-tâches** à un niveau : un parent affiche son avancement (« 2/5 »), cocher la dernière propose de clore le parent, cocher le parent emporte les siennes, et une série qui revient repart avec ses cases. **Ordre manuel** au glisser-déposer (à la poignée, sur téléphone comme au clavier) : la journée du jour se range comme on la fera (l'ordre choisi passe devant l'heure, et l'accueil le suit), comme les checklists, les tâches sans date et les sous-tâches ; « En retard » et « À venir » restent classés par date, où elle est l'information utile. Vue **« À moi »** : tout ce qui m'est affecté, toutes listes confondues. Cocher est un tap, sans confirmation ; supprimer, reporter, réordonner et « Tout reporter à aujourd'hui » s'**annulent** quelques secondes. Le retard se dit, il ne crie pas. **Écriture tâche par tâche** : deux téléphones peuvent cocher en même temps sans que l'un décoche l'autre, et les gestes faits **hors ligne** repartent au retour du réseau. Voir [`docs/taches.md`](docs/taches.md). |
...
| 🗓️ **Emploi du temps** | La semaine type du foyer. Un créneau porte **un ou plusieurs membres** : la messe du dimanche ou un trajet en voiture font une ligne, pas quatre. **Sans filtre, la vue montre tout le foyer** ; chaque créneau porte la pastille et les initiales des personnes concernées, le débordement étant compté au-delà de trois. Le filtre par membre est un affinage à sélection multiple, visible en permanence tant qu'il est actif et effaçable en un geste. Vue jour sur téléphone, semaine sur écran large, en listes triées par heure : les créneaux qui se chevauchent restent lisibles. **Copie de journée** : on copie un jour et on le colle sur plusieurs autres d'un coup, en **fusionnant** (rien n'est détruit, aucun doublon) ou en **remplaçant**. Dès que la cible n'est pas vide, un aperçu dit ce qui sera ajouté, remplacé et supprimé, en nommant les créneaux ; après coup, un message dit ce qui a été fait et propose d'annuler. L'annulation vise les créneaux un par un, elle n'écrase donc jamais ce que l'autre appareil vient d'écrire. On peut aussi **coller pour un autre membre** (mêmes horaires, autre personne) et **dupliquer** un créneau. **Récurrence sobre** : un créneau est hebdomadaire ou ponctuel, avec une **période de validité** (l'activité démarre en septembre et s'arrête en juin) et un filtre **période scolaire / vacances** alimenté par les vacances de votre académie et les jours fériés calculés. La vue est datée, semaine par semaine. Modifier ou supprimer une occurrence demande **ce jour seulement, à partir de cette date, ou toute la série**, et les trois s'annulent. Quand la source des vacances est indisponible, les créneaux sont **affichés** plutôt que cachés. **Saisie** : les **temps libres** d'une journée apparaissent entre les créneaux, et taper dessus crée un créneau à cette heure-là, jour et heure déjà remplis. Le champ intitulé propose ce que le foyer écrit déjà. À la souris, un créneau se **glisse** d'un jour à l'autre, en demandant s'il s'agit de la série ou d'une seule fois ; sur téléphone le même déplacement passe par le formulaire, faute de glisser tactile fiable. **Il alimente les autres modules** : une case « hors du foyer » sur chaque créneau donne le nombre de **couverts** au planning des repas, qui ne redemande plus l'information dans une grille tenue à la main. La tuile d'accueil montre le jour **et le lendemain**. Voir [`docs/emploi-du-temps.md`](docs/emploi-du-temps.md). |
...
| 💡 **Aide à la composition** | **Couverts déduits de l'emploi du temps** : un créneau marqué « hors du foyer » qui couvre l'heure d'un repas retire ceux qu'il concerne, et les quantités de courses suivent, créneau par créneau. Rien à ressaisir ailleurs. Dérogation ponctuelle en un tap dans la modale d'un repas. **Suggestions explicables** pour un créneau vide : « jamais encore faite », « pas faite depuis trois semaines », « 2 ingrédients déjà sur la liste », « prête en 15 min ». Jamais un score opaque, aucun appel à un service extérieur. **Anti-répétition sur quinze jours**, et les recettes qui ne conviennent pas à un convive attendu sont écartées, en le disant. |
...
| ⚙️ **Paramètres** | Thème, membres, rappels sur cet appareil (Web Push, avec envoi de test et journal des envois), partage du calendrier, mises à jour. |
...
Modèle, opérations ciblées et journal anti-rejeu, récurrence (les deux modes,
la tolérance, occurrence ou série), rappels par Web Push (ce qu'il faut faire
sur iPhone, ce qui casse, l'écran d'état, l'exploitation), liens avec le reste
du foyer (contrat, document, courses, emploi du temps, agenda et option ICS),
sous-tâches à un niveau et ordre manuel, hors ligne (ce qui est garanti, ce qui
ne l'est pas), migration 9 avec sauvegarde, vérification et retour en arrière en
commandes shell, lignes de journal à savoir lire :
