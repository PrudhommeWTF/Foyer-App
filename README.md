...
| 🍽️ **Repas** | Déjeuner et dîner (petit-déjeuner en option dans *Paramètres → Repas*), en **fenêtre de 3 jours ou de 7 jours**. **Grille sur grand écran, pile de jours sur téléphone** : chaque créneau est une ligne pleine largeur, rien ne défile latéralement, et la semaine s'ouvre sur le jour même. La génération des courses suit la fenêtre affichée. **Plusieurs plats par créneau** : entrée, plat, dessert se choisissent séparément, chacun étant une recette du carnet ou un texte libre. **Couverts par créneau** : les quantités de la liste de courses suivent le nombre de convives. |
