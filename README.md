...
| 🍽️ **Repas** | Semaine courante, déjeuner et dîner (petit-déjeuner en option dans *Paramètres → Repas*). **Grille 7 jours sur grand écran, pile de jours sur téléphone** : chaque créneau est une ligne pleine largeur, la semaine s'ouvre sur le jour même et rien ne défile latéralement. **Plusieurs plats par créneau** : entrée, plat, dessert se choisissent séparément, chacun étant une recette du carnet ou un texte libre. **Couverts par créneau** : les quantités de la liste de courses suivent le nombre de convives. |
