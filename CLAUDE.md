...
- **Document JSON pour le cœur, tables relationnelles pour les Finances.** L'essentiel de l'état du foyer (membres, événements, tâches, courses, repas, recettes, contacts, cartes de fidélité, réglages) est un **unique document JSON** (`HouseholdState`) stocké dans SQLite, exposé par `GET/PUT /api/state`. Le module **Finances** fait exception : il vit dans ses propres **tables relationnelles** (`fin_*`), hors du document, pour ne pas recharger et réécrire des milliers d'opérations à chaque sauvegarde. Les **pièces jointes** (photos de recettes, pièces Finances) sont des octets sur le disque adressés par des tables (`hh_attachments`, `fin_attachments`), jamais dans le document. **Courses, tâches et lieux** vivent dans le document mais s'écrivent par **opérations ciblées journalisées** (`hh_shop_ops`, `hh_task_ops`, `hh_place_ops`), pas par un PUT complet, pour que deux téléphones qui cochent en même temps ne s'écrasent pas. Les **lieux** (inventaires de vacances : `places` + `placeItems`) suivent le même dispositif, un seul flux d'opérations portant à la fois les lieux et leurs affaires (voir `docs/lieux.md`).
...
backend/src/  # server.ts (Express + routes), db.ts (SQLite), models.ts, seed.ts (EMPTY_STATE + buildInitialState)
              # shopping/, tasks/ et places/ : sous-arbres du document écrits par opérations ciblées (ops.ts pur, repo.ts transaction + journal, routes.ts)
...
- **Calendrier partagé** en lecture seule via flux **ICS** (`/api/calendar/feed.ics?token=…`) : événements, échéances de contrat, occurrences des créneaux d'emploi du temps **publiés à l'agenda** (`sched[].sync`, dérivées côté serveur par `publishedSlotOccurrences`), et les tâches datées sur option (`settings.icsTasks`). Pas de CalDAV.
...
- **Autocomplétion du lieu** d'un événement : le backend relaie la **Base Adresse Nationale** (`api-adresse.data.gouv.fr`, hôte fixe, gratuit, sans clé), voir `backend/src/places.ts`. Coupable par le réglage `placeSuggest` (défaut activé) ; dégradation silencieuse (le champ reste une saisie libre).
